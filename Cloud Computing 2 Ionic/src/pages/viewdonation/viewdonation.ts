...
import { ViewDonationDetailsPage } from '../ViewDonationDetails/ViewDonationDetails';
...
export class ViewdonationPage {
  ...
  ionViewDidLoad() {
	...
	getdonations()
		 {
          ...
          value => {
                        if(value != null && value.hasOwnProperty('USER_NAME') && value.hasOwnProperty('SESSION_KEY') && value.hasOwnProperty('USER_TYPE'))
                        {
                            ...
                            objdataSession['usertype'] = value.USER_TYPE;
							if(this.loaded==0){
								this.connectServer.getData("viewdonate",objdataSession).then(
								response => { 
										   if(response!=null)
												{	
												console.log(response);
												if(response['response'][0]['success'] == "1")
													{
														this.message = response['response'][1]['message'];
														this.donation_array = response['response'][5]['viewdonate'];
														this.loaded = 1;
														this.filterData = response['response'][5]['viewdonate'];
													}
												}
											}
									);
	...
	showRequestDetails(item) {
		console.log(item);
		this.navCtrl.push(ViewDonationDetailsPage, {item: item});
	}
